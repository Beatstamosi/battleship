...
function simulateClickAndObserve(attackField) {
  return new Promise((resolve, reject) => {
    ...
    if (
      attackField.classList.contains("hit") ||
    ...
    if (
      typeof attackField.click !== "function" ||
...
export const grimhollow = {
  ...
  attack: async function (availableTargets) {
      ...
      return availableTargets.find(
        (item) =>
          parseInt(item.dataset.column) ===
            parseInt(attackField.dataset.column) - 1 &&
    ...
    const getAttackTop = (attackField, availableTargets) => {
      return availableTargets.find(
        (item) =>
          parseInt(item.dataset.row) ===
            parseInt(attackField.dataset.row) - 1 &&
    ...
    try {
      ...
      if (result == "hit") {
        ...
        if (this.hitCounter >= 2) {
          ...
          const nextTargets = availableTargets.filter(
            (field) =>
              field.gameField?.ship &&
              field.gameField.ship.length === gameField.ship.length &&
          ...
          adjacentTargets.forEach((target) => {
            if (
              !this.nextAttack.includes(target) &&
              !this.alreadyAttacked.includes(target)
            ) {
...
export const boneshard = {
  ...
  attack: async function (availableTargets) {
    ...
    attackField =
      this.nextAttack.length === 0
        ? availableTargets[getRandomIndex(availableTargets)]
        : (attackField = this.nextAttack.shift());
    ...
    try {
      ...
      if (result === "hit" && gameField?.ship) {
        let nextTargets = availableTargets.filter(
          (field) =>
            field.gameField?.ship &&
            field.gameField.ship.length === gameField.ship.length,
        );
